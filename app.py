...
class GraphNode(BaseModel):
    id: str
    label: str
    type: str  # "concept", "entity", or "process"
...
app = FastAPI(
    ...
    description="R&D Knowledge Management System API",
    version="1.0.0"
)
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # In production, replace with your Replit domain
...
@app.post("/api/facts")
def create_fact(fact: Fact):
    """Create a new fact"""
    global fact_id_counter
    
    fact.id = fact_id_counter
    fact_id_counter += 1
    
    facts_db.append(fact.dict())
    return {"success": True, "fact": fact}

@app.patch("/api/facts/{fact_id}")
def update_fact(fact_id: int, fact: Fact):
    """Update an existing fact"""
    for i, f in enumerate(facts_db):
        if f["id"] == fact_id:
            fact.id = fact_id
            facts_db[i] = fact.dict()
            return {"success": True, "fact": fact}
...
def delete_fact(fact_id: int):
    """Delete a fact"""
    global facts_db
    facts_db = [f for f in facts_db if f["id"] != fact_id]
    return {"success": True, "message": f"Fact {fact_id} deleted"}

@app.get("/api/graph")
def get_graph():
    """Get graph nodes and edges derived from facts"""
    nodes = []
    edges = []
    node_set = set()
...
async def upload_documents(files: List[str]):
    """Upload documents (placeholder - implement your document processing)"""
    # TODO: Implement actual document upload and processing
    return {
        "success": True,
        "message": f"Received {len(files)} files",
        "document_ids": [f"doc_{i}" for i in range(len(files))]
    }

@app.post("/api/process")
def process_documents(document_ids: List[str]):
    """Process documents for knowledge extraction (placeholder)"""
    # TODO: Implement your knowledge extraction logic
    return {
        "success": True,
...
def chat(message: ChatMessage):
    """Chat with LLM about the knowledge base (placeholder)"""
    # TODO: Implement your LLM chat logic
    response = f"You asked: {message.message}. This is a placeholder response."
    return {
        "success": True,
        "response": response
    }
...
def health():
    ...
    return {"status": "healthy", "facts_count": len(facts_db)}

# ========================================
# GRADIO INTERFACE (Your existing UI)
# ========================================

# Keep your existing Gradio interface here
# This is just an example - replace with your actual Gradio app
...
with gr.Blocks(title="Research Brain") as demo:
    ...
    gr.Markdown("Your existing Gradio interface goes here")
    
    text_input = gr.Textbox(label="Input")
    output = gr.Textbox(label="Output")
    btn = gr.Button("Submit")
    btn.click(gradio_predict, inputs=text_input, outputs=output)
    
    gr.Markdown("""
    ### API Endpoints
    - **Knowledge Base**: `/api/knowledge-base` - Get all facts
    - **Create Fact**: `POST /api/facts` - Add new fact
    - **Graph Data**: `/api/graph` - Get graph visualization data
    - **API Docs**: `/docs` - Full API documentation
    """)

# Mount Gradio app to FastAPI at /gradio path
